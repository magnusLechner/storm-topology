 ...
 */
package at.storm.commons.wordnet;
...
public class WordNet {
	...
	private WordNet() {
		try {
			...
			m_wordNetDir = new File(Configuration.TEMP_DIR_PATH + File.separator + "dict");
			...
			m_dict.load(true);
			LOG.info("Loaded Wordnet into memory in " + (System.currentTimeMillis() - t) + " msec");
	...
	public synchronized POS findPOS(String word) {
		...
		for (POS pos : POS.values()) {
			// From JavaDoc: The surface form may or may not contain whitespace
			// or
