 ...
 */
package at.storm.commons.util.io;
...
public class FileUtils {
	...
	public static Map<String, String> readFile(InputStream is, String splitRegex, boolean logging) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (logging) {
					LOG.info("Add entry key: '" + values[0].trim() + "' value: '" + values[1].trim() + "'");
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static Map<String, Double> readFile(String file, String splitRegex, boolean containsPOSTags,
			boolean featureScaling, double minValue, double maxValue) {
		return readFile(IOUtils.getInputStream(file), splitRegex, containsPOSTags, featureScaling, minValue, maxValue,
				false);
	}
	...
	public static Map<String, Double> readFile(InputStream is, String splitRegex, boolean containsPOSTags,
			boolean featureScaling, double minValue, double maxValue, boolean logging) {
...
//				i++;
				...
				if (featureScaling) {
					...
					if (logging) {
						LOG.info("Add entry key: '" + key + "' value: '" + value + "' normalizedValue: '"
								+ normalizedValue + "'");
					}
		...
		if (featureScaling) {
			if (minValue != actualMinValue) {
				LOG.error("minValue is incorrect! actual minValue: " + actualMinValue + " given minValue: " + minValue
						+ " (normalized values might be wrong!)");
			}
			if (maxValue != actualMaxValue) {
				LOG.error("maxValue is incorrect! actual maxValue: " + actualMaxValue + " given maxValue: " + maxValue
						+ " (normalized values might be wrong!)");
			}
		}
		return map;
	...
	public static WordListMap<Double> readWordListMap(String file, String splitRegex, boolean containsPOSTags,
			boolean featureScaling, double minValue, double maxValue) {
		return readWordListMap(IOUtils.getInputStream(file), splitRegex, containsPOSTags, featureScaling, minValue,
				maxValue, false);
	}
	...
	public static WordListMap<Double> readWordListMap(InputStream is, String splitRegex, boolean containsPOSTags,
			boolean featureScaling, double minValue, double maxValue, boolean logging) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (featureScaling) {
					...
					if (logging) {
						LOG.info("Add Key: '" + key + "' Value: '" + value + "' normalizedValue: '" + normalizedValue
								+ "'");
					}
			...
			LOG.info("Loaded " + wordListMap.size() + " items [minValue: " + actualMinValue + ", maxValue: "
					+ actualMaxValue + "]");

			if (featureScaling) {
				if (minValue != actualMinValue) {
					LOG.error("minValue is incorrect! actual minValue: " + actualMinValue + " given minValue: "
							+ minValue + " (normalized values might be wrong!)");
				}
				if (maxValue != actualMaxValue) {
					LOG.error("maxValue is incorrect! actual maxValue: " + actualMaxValue + " given maxValue: "
							+ maxValue + " (normalized values might be wrong!)");
				}
			}
