...
public class RegexUtils {
	...
	// URL Regex
	public static final String URL = "(?:" + "(?i)(https?|ftp)://" + "(-\\.)?" + "([^\\s/?\\.#-]+\\.?)*(/[^\\s\\.]*)?"
			+ ")";
	...
	// Phone Regex
	public static final String PHONE = "(?:" + "(?:\\+?[01][\\-\\s.]*)?" + "(?:[\\(]?\\d{3}[\\-\\s.\\)]*)?" + "\\d{3}"
			+ "[\\-\\s.]*" + "\\d{4}" + ")";
	...
	// Hashtag Regex
	public static final String HASH_TAG = "(?:" + "\\#+([A-Za-z]+[A-Za-z0-9_\\'\\-]*[A-Za-z0-9_]+)" + ")";
	public static final Pattern HASH_TAG_PATTERN = Pattern.compile(HASH_TAG);

	// Retweet Regex
	public static final String RETWEET = "(?:" + "(?i)(RT|retweet|from|via)" + "((?:\\b\\W*\\@\\w+)*)" + ")";
	...
	public static final String CONTAINS_HTML_SYMBOLS = "(?:" + "&#[0-9]{2,4};)" + "|" + "(?:&[a-zA-Z0-9]{2,6};" + ")";
	public static final Pattern CONTAINS_HTML_SYMBOLS_PATTERN = Pattern.compile(CONTAINS_HTML_SYMBOLS);

	// Contains Unicode symbols
	public static final String CONTAINS_UNICODE_SYMBOLS = "(?:" + "[^\\u0000-\\u007F]+" + ")" + "|" + "(?:"
			+ "\\\\u[0-9a-fA-F]{4,5}+" + ")";
	public static final Pattern CONTAINS_UNICODE_SYMBOLS_PATTERN = Pattern.compile(CONTAINS_UNICODE_SYMBOLS);
	...
	public static final Pattern TWO_OR_MORE_REPEATING_CHARS_PATTERN = Pattern.compile(TWO_OR_MORE_REPEATING_CHARS);
	...
	public static final Pattern THREE_OR_MORE_REPEATING_CHARS_PATTERN = Pattern.compile(THREE_OR_MORE_REPEATING_CHARS);

	// Punctuation Regex
	public static final String PUNCTUATIONS = "^[\\p{Punct}\\s]+$";
	public static final Pattern PUNCTUATIONS_PATTERN = Pattern.compile(PUNCTUATIONS);

	// Underscore Regex
	public static final String UNDERSCORES = "^_+$";
	public static final Pattern UNDERSCORES_PATTERN = Pattern.compile(UNDERSCORES);

	// Starts with an alphabetic character
	public static final String STARTS_WITH_ALPHABETIC_CHAR = "^[a-zA-Z].*$";
	public static final Pattern STARTS_WITH_ALPHABETIC_CHAR_PATTERN = Pattern.compile(STARTS_WITH_ALPHABETIC_CHAR);
	...
	public static final String SPECIAL_NUMBER = "(?:" + "\\$?" + "[+\\-]?" + "\\d+" + "([\\.|\\,|\\:|\\-]\\d+)*" + "(?:"
			+ "(?i)(%|fm|am|pm|p|lb)" + "|" + "(c|p|st|nd|rd|th)" + ")?" + ")";
	public static final Pattern SPECIAL_NUMBER_PATTERN = Pattern.compile(SPECIAL_NUMBER);

	// Separated number Regex
	public static final String SEPARATED_NUMBER = "(?:" + "\\d+" + "(?:" + "(?i)(am|pm)" + "|" + "(c|st|nd|rd|th)"
			+ ")?" + "[\\/|\\,|\\-]+" + "\\d+" + "([\\/|\\,|\\-]\\d+)*" + "(?:" + "(?i)(%|fm|am|pm|lb)" + "|"
			+ "(c|p|st|nd|rd|th)" + ")?" + ")";
	public static final Pattern SEPARATED_NUMBER_PATTERN = Pattern.compile(SEPARATED_NUMBER);

	// Punctuation between words
	public static final String PUNCTUATION_BETWEEN_WORDS = "^(.*[^\\.|\\,|\\!|\\/|\\-])" + "[\\.|\\,|\\!|\\/|\\-]+"
			+ "([^\\.|\\,|\\!|\\/|\\-].*)$";
	public static final Pattern PUNCTUATION_BETWEEN_WORDS_PATTERN = Pattern.compile(PUNCTUATION_BETWEEN_WORDS);

	// Alternating letter dot pattern e.g., L.O.V.E
	public static final String ALTERNATING_LETTER_DOT = "[a-zA-Z]\\.(?:[a-zA-Z](\\.)?)+";
	public static final Pattern ALTERNATING_LETTER_DOT_PATTERN = Pattern.compile(ALTERNATING_LETTER_DOT);

	// Words with apostrophes or dashes
	public static final String WORDS_WITH_APOSTROPHES_DASHES = "(?:" + "[a-zA-Z][a-zA-Z\\'\\-\\_]+[a-zA-Z]" + ")";

	// Words without apostrophes or dashes
	public static final String WORDS_WITHOUT_APOSTROPHES_DASHES = "(?:" + "[\\w_]+" + ")";
	...
	private static final String EMOTICON_MOUTH = "[0]?" // mouth
			+ "([\\(\\)\\[\\]\\/\\\\}{*.^<>=@|,bdDpPLScoO$X#J3&]" + ")\\1{0,}";
	public static final String EMOTICON = "(?:" + "[<>oO0}3|]?" + EMOTICON_EYES + EMOTICON_NOSE + EMOTICON_MOUTH
			+ "|" /* reverse */
			+ EMOTICON_MOUTH + EMOTICON_EYES + "[<>]?" + ")";
	private static final String EMOTICON_DELIMITER = SPACE_REGEX + "|" + PUNCTUATION_REGEX;
	public static final Pattern EMOTICON_PATTERN = Pattern
			.compile("(?<=^|" + EMOTICON_DELIMITER + ")" + EMOTICON + "(?=$|" + EMOTICON_DELIMITER + ")");
