 ...
 */
package at.storm.commons.tfidf;
...
public class TfIdf {
	...
	private static final boolean LOGGING = Configuration.get("commons.tfidf.logging", false);

	public static <T> Map<T, Double> tf(Collection<T> document) {
		return tf(new HashMap<T, Double>(), document);
	}

	public static <T> Map<T, Double> tf(Map<T, Double> termFreq, Collection<T> terms) {
	...
	public static <T> Map<T, Double> normalizeTf(Map<T, Double> termFreq, TfType type) {
	...
	public static <T> Map<T, Double> idf(List<Map<T, Double>> termFreq) {
		...
		for (Map.Entry<T, Long> e : docFreq.entrySet()) {
			...
			if (LOGGING) {
				LOG.info("term: " + term.toString() + " idf: log(" + totalDocuments + "/" + documentFreq + ") + 1 = "
	...
	public static <T> Map<T, Double> tfIdf(Map<T, Double> termFreq, Map<T, Double> inverseDocFreq,
