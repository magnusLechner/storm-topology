 ...
 */
package at.storm.commons.tfidf.pos;
...
import at.storm.commons.wordnet.POSTag;
...
public class TweetTfIdf {
	...
	public static List<Map<String, Double>> tfTaggedTokenTweets(List<List<TaggedToken>> tweets, TfType type,
			boolean usePOSTags) {
	...
	public static Map<String, Double> tfFromTaggedTokens(List<TaggedToken> tweet, TfType type, boolean usePOSTags) {
		...
		for (TaggedToken taggedToken : tweet) {
			...
			if ((!arkTag.equals(",")) && (!arkTag.equals("$")) && (!arkTag.equals("G")) && (!arkTag.equals("@"))
					&& (!arkTag.equals("~")) && (!arkTag.equals("U")) && (!stopWords.isStopWord(word))) {
				...
				// LOG.info("word: '" + word + "' pennTag: '" + pennTag + "'
				// tag: '"
				// + posTag + "'");
