...
public class EmoticonFeatureVectorGenerator extends FeatureVectorGenerator {
	private static final Logger LOG = LoggerFactory.getLogger(EmoticonFeatureVectorGenerator.class);
	...
	public EmoticonFeatureVectorGenerator() {
		this.m_emoticonDict = EmoticonDictionary.getInstance();
		LOG.info("VectorSize: " + getFeatureVectorSize());
	}

	public EmoticonFeatureVectorGenerator(int vectorStartId) {
		this();
	...
	public Map<Integer, Double> generateFeatureVector(List<TaggedToken> taggedTokens) {
		...
		Map<Integer, SentimentResult> tweetSentiments = m_emoticonDict.getSentenceEmoticonSentiment(taggedTokens);
