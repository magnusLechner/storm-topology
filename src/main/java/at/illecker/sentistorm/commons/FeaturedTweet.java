...
public final class FeaturedTweet extends Tweet implements Serializable {
	...
	public FeaturedTweet(long id, String text, double score, List<String> tokens, List<String> preprocessedTokens,
	...
	public FeaturedTweet(long id, String text, double score, Map<Integer, Double> featureVector) {
		super(id, text, score);
		m_featureVector = featureVector;
		m_tokens = null;
		m_preprocessedTokens = null;
		m_taggedTokens = null;
	}
	
	public FeaturedTweet(Tweet tweet, List<String> tokens, List<String> preprocessedTokens,
			List<TaggedToken> taggedTokens, Map<Integer, Double> featureVector) {
	...
	public FeaturedTweet(Tweet tweet, List<String> tokens, List<String> preprocessedTokens,
			Map<Integer, Double> featureVector) {
		this(tweet.getId(), tweet.getText(), tweet.getScore(), tokens, preprocessedTokens, featureVector);
	}
	
	//added for Aggregation
	public FeaturedTweet(Tweet tweet, Map<Integer, Double> featureVector) {
		this(tweet.getId(), tweet.getText(), tweet.getScore(), featureVector);
	}
	...
	public static final List<List<TaggedToken>> getTaggedTokensFromTweets(List<FeaturedTweet> featuredTweets) {
	...
	public static final List<FeaturedTweet> generateFeatureTweets(List<Tweet> tweets) {
		...
		// POS Tagging
		List<List<TaggedToken>> taggedTweets = postTagger.tagTweets(preprocessedTweets);
		...
		TweetTfIdf tweetTfIdf = TweetTfIdf.createFromTaggedTokens(taggedTweets, TfType.LOG, TfIdfNormalization.COS,
				true);
		FeatureVectorGenerator fvg = new CombinedFeatureVectorGenerator(true, tweetTfIdf);
	...
	public static void main(String[] args) {
		...
		List<FeaturedTweet> featuredTrainTweets = generateFeatureTweets(dataset.getTrainTweets(includeDevTweets));
